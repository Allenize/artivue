...
import { LayoutDashboard, Image, Users, Users2, LogOut, Plus, Edit2, Trash2, X, Save, Check, Eye, ThumbsUp, Star, Home, Menu } from 'lucide-react'
  ...
  return (
    <div style={{ marginBottom: '.65rem' }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <input className="field" placeholder={`New ${label.toLowerCase()}…`} value={newItem}
...
function StatCard({ label, value, icon: Icon, color, sub }) {
  return (
    <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '1rem 1.2rem', display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div>
        <div style={{ fontSize: 26, fontFamily: 'Cormorant Garamond,serif', color: 'var(--dark-text)', lineHeight: 1 }}>{value}</div>
        <div style={{ fontSize: 10, color: 'var(--light-text)', marginTop: 2 }}>{label}</div>
        {sub && <div style={{ fontSize: 9, color: 'var(--accent-gold)', marginTop: 1 }}>{sub}</div>}
      </div>
    </div>
  )
}
...
function ArtworkForm({ artwork, artists, categories, movements, onSave, onCancel }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '1.2rem', marginBottom: '1rem' }}>
      <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 18, fontStyle: 'italic', color: 'var(--dark-text)', marginBottom: '1rem' }}>
        {artwork ? 'Edit Artwork' : 'New Artwork'}
      </div>
      <ImageUpload value={f.image} onChange={v => set('image', v)} label="Artwork Image" height={150} />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: '.6rem' }}>
        ...
        <select className="field" value={f.movement} onChange={e => set('movement', e.target.value)} style={{ marginBottom: 0 }}>
          {movements.map(m => <option key={m}>{m}</option>)}
        </select>
        <select className="field" value={f.category} onChange={e => set('category', e.target.value)} style={{ marginBottom: 0 }}>
      ...
      <textarea className="field" placeholder="Description *" value={f.description} onChange={e => set('description', e.target.value)} style={{ marginTop: '.6rem', marginBottom: 0 }} />
      <textarea className="field" placeholder="Fun fact (Did you know?)" value={f.fact} onChange={e => set('fact', e.target.value)} style={{ marginTop: '.6rem', marginBottom: 0, minHeight: 55 }} />
      <input className="field" placeholder="Tags (comma separated)" value={f.tags} onChange={e => set('tags', e.target.value)} style={{ marginTop: '.6rem', marginBottom: 0 }} />
      <div style={{ marginTop: '.8rem' }}>
        <div style={{ fontSize: 10, color: 'var(--light-text)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 6 }}>Background Gradient</div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {GRADIENTS.map((g, i) => <div key={i} onClick={() => set('gradient', g)} style={{ width: 40, height: 26, borderRadius: 6, background: g, cursor: 'pointer', border: f.gradient === g ? '2.5px solid var(--accent-rust)' : '2.5px solid transparent', transition: 'border .2s' }} />)}
        </div>
      </div>
...
function ArtistForm({ artist, onSave, onCancel }) {
  const [f, setF] = useState(artist || { name: '', short: '', initials: '', color: COLORS[0], era: '', nationality: '', bio: '', image: null })
  const set = (k, v) => setF(p => ({ ...p, [k]: v }))
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <ImageUpload value={f.image} onChange={v => set('image', v)} label="Artist Profile Image" height={130} />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: '.6rem' }}>
      ...
      <textarea className="field" placeholder="Biography *" value={f.bio} onChange={e => set('bio', e.target.value)} style={{ marginTop: '.6rem', marginBottom: 0 }} />
      <div style={{ marginTop: '.8rem' }}>
...
export default function AdminScreen() {
  ...
  // Load persisted categories/movements from Firestore
  useEffect(() => {
    getSettings().then(s => {
      if (s.categories?.length) setCategories(s.categories)
      if (s.movements?.length) setMovements(s.movements)
    })
  }, [])

  // Save categories/movements to Firestore whenever they change
  const handleSaveSettings = async () => {
    try {
      await saveSettings({ categories, movements })
      setSettingsSaved(true)
      addNotification('Categories & movements saved! ✅')
      setTimeout(() => setSettingsSaved(false), 3000)
    } catch(e) {
      addNotification('Failed to save settings. Check Firestore rules.')
    }
  }
  ...
  const SidebarContent = () => (
    <div style={{ width: '100%', height: '100%', background: 'var(--sidebar-bg)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '1.5rem 1.2rem 1rem', borderBottom: '1px solid rgba(237,230,214,.08)', display: 'flex', alignItems: 'center', gap: 10 }}>
        <ArtivueLogo size={28} />
        ...
        <button onClick={() => setSidebarOpen(false)} className="admin-close-btn" style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'rgba(237,230,214,.4)', cursor: 'pointer', display: 'none' }}><X size={16} /></button>
      </div>
      <div style={{ padding: '.5rem 0', flex: 1, overflowY: 'auto' }}>
        {TABS.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => { setTab(id); setSidebarOpen(false) }}
            style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '11px 1.2rem', background: tab === id ? 'rgba(237,230,214,.06)' : 'none', border: 'none', borderLeft: `2px solid ${tab === id ? 'var(--accent-gold)' : 'transparent'}`, color: tab === id ? 'var(--sidebar-text)' : 'rgba(237,230,214,.4)', fontSize: 12, fontFamily: 'Jost,sans-serif', letterSpacing: '.06em', cursor: 'pointer', transition: 'all .2s', textAlign: 'left' }}>
      ...
      <div style={{ padding: '1rem 1.2rem', borderTop: '1px solid rgba(237,230,214,.08)', display: 'flex', flexDirection: 'column', gap: 6 }}>
        <button onClick={() => navigate('/home')} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', background: 'rgba(237,230,214,.06)', border: '1px solid rgba(237,230,214,.1)', borderRadius: 8, padding: '7px 12px', cursor: 'pointer', color: 'rgba(237,230,214,.7)', fontSize: 11, fontFamily: 'Jost,sans-serif', letterSpacing: '.08em' }}>
      ...
      <style>{`.admin-close-btn { display: none !important; } @media(max-width:768px){ .admin-close-btn { display: flex !important; } }`}</style>
    </div>
  )
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100dvh', background: 'var(--beige)' }}>
      {/* Desktop sidebar */}
      <div className="admin-side" style={{ width: 220, background: 'var(--sidebar-bg)', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 50 }}>
        <SidebarContent />
      </div>
        ...
        {sidebarOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 98, backdropFilter: 'blur(2px)' }}
              onClick={() => setSidebarOpen(false)} />
            <motion.div initial={{ x: -220 }} animate={{ x: 0 }} exit={{ x: -220 }}
              transition={{ type: 'spring', stiffness: 300, damping: 30 }}
              style={{ position: 'fixed', left: 0, top: 0, bottom: 0, width: 220, zIndex: 99 }}>
              <SidebarContent />
            </motion.div>
          </>
        )}
      ...
      {/* Main content */}
      <div className="admin-main" style={{ flex: 1, marginLeft: 220, padding: '0', minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
        {/* Mobile top bar */}
        <div className="admin-topbar" style={{ display: 'none', padding: '12px 16px', background: 'var(--sidebar-bg)', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 40 }}>
          <button onClick={() => setSidebarOpen(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--sidebar-text)' }}><Menu size={20} /></button>
          <ArtivueLogo size={24} />
          <span style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 14, color: 'var(--sidebar-text)', letterSpacing: '.1em', textTransform: 'uppercase', flex: 1 }}>Admin Panel</span>
          <button onClick={() => navigate('/home')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(237,230,214,.5)', fontSize: 10, fontFamily: 'Jost,sans-serif', letterSpacing: '.06em', display: 'flex', alignItems: 'center', gap: 4 }}>
            <Home size={12} /> App
          </button>
        </div>
        ...
        <div style={{ padding: '1.5rem', flex: 1 }}>
          ...
          {tab === 'dashboard' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 24, fontWeight: 300, color: 'var(--dark-text)', marginBottom: '1.2rem' }}>Dashboard</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: '.8rem', marginBottom: '1.5rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: '1rem' }}>
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '1rem' }}>
                  <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--dark-text)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: 6 }}><Eye size={11} />Most Viewed</div>
                  {topViewed.map(art => (
                    <div key={art.id} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 13, fontStyle: 'italic', color: 'var(--dark-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{art.title}</div>
                        <div style={{ fontSize: 9, color: 'var(--light-text)' }}>{art.artist}</div>
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--mid-text)', fontWeight: 500, flexShrink: 0 }}>{art.views || 0}</div>
                    </div>
                  ))}
                </div>
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '1rem' }}>
                  <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--dark-text)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: 6 }}><ThumbsUp size={11} />Most Liked</div>
                  {topLiked.map(art => (
                    <div key={art.id} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                      <div style={{ width: 34, height: 34, borderRadius: 6, overflow: 'hidden', flexShrink: 0 }}>
                        {art.image ? <img src={art.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', background: art.gradient }} />}
                      ...
                      <div style={{ fontSize: 12, color: 'var(--mid-text)', fontWeight: 500, flexShrink: 0 }}>{art.likes?.length || 0}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {tab === 'artworks' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '1rem', marginBottom: '1rem' }}>
                ...
                <button onClick={handleSaveSettings} className="btn btn-primary" style={{ marginTop: '0.8rem' }}>
                  <Save size={12} />{settingsSaved ? '✅ Saved!' : 'Save Categories & Movements'}
                </button>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: 8 }}>
                <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 22, fontWeight: 300, color: 'var(--dark-text)' }}>Artworks <span style={{ fontSize: 13, fontStyle: 'normal', color: 'var(--light-text)' }}>({artworks.length})</span></div>
                ...
                {artworks.map(art => (
                  <div key={art.id}>
                    ...
                    {artworkForm?.id !== art.id && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', gap: 10, alignItems: 'center', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '10px 12px', flexWrap: 'wrap' }}>
                        <div style={{ width: 50, height: 50, borderRadius: 8, overflow: 'hidden', flexShrink: 0 }}>
                          {art.image ? <img src={art.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', background: art.gradient }} />}
                        </div>
                        <div style={{ flex: 1, minWidth: 120 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                            <span style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 14, fontStyle: 'italic', color: 'var(--dark-text)' }}>{art.title}</span>
                            {art.featured && <span className="badge badge-gold"><Star size={9} />Featured</span>}
                          </div>
                          <div style={{ fontSize: 10, color: 'var(--light-text)' }}>{art.artist} · {art.year} · {art.category}</div>
                          <div style={{ fontSize: 9, color: 'var(--mid-text)', marginTop: 2 }}>{art.views || 0} views · {art.likes?.length || 0} likes</div>
                        </div>
          ...
          {tab === 'artists' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: 8 }}>
                <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 22, fontWeight: 300, color: 'var(--dark-text)' }}>Artists <span style={{ fontSize: 13, fontStyle: 'normal', color: 'var(--light-text)' }}>({artists.length})</span></div>
                ...
                {artists.map(artist => (
                  <div key={artist.id}>
                    ...
                    {artistForm?.id !== artist.id && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', gap: 10, alignItems: 'center', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '10px 12px' }}>
                        <div style={{ width: 44, height: 44, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, border: '2px solid var(--border)' }}>
                          {artist.image ? <img src={artist.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', background: artist.color || '#8B5E3C', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Cormorant Garamond,serif', fontSize: 14, color: '#fff', fontStyle: 'italic' }}>{artist.initials}</div>}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 14, fontStyle: 'italic', color: 'var(--dark-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{artist.name}</div>
          ...
          {tab === 'community' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 22, fontWeight: 300, color: 'var(--dark-text)', marginBottom: '1.2rem' }}>Community Moderation</div>
              {pending.length > 0 && (
                <>
                  ...
                  {pending.map(post => (
                    <motion.div key={post.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      style={{ display: 'flex', gap: 10, background: 'var(--card-bg)', border: '1px solid rgba(196,98,45,.3)', borderRadius: 'var(--r-md)', padding: '12px', marginBottom: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                      <div style={{ width: 54, height: 54, borderRadius: 8, overflow: 'hidden', flexShrink: 0, background: post.gradient || 'var(--warm-tan)' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 100 }}>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--mid-text)' }}>by {post.userName}</div>
                        <div style={{ fontSize: 10, color: 'var(--light-text)', marginTop: 2 }}>{post.description?.substring(0, 80)}</div>
                      </div>
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--dark-text)', margin: '1.2rem 0 .8rem' }}>Approved Posts ({approved.length})</div>
              {approved.map(post => (
                <motion.div key={post.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  style={{ display: 'flex', gap: 10, background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '10px 12px', marginBottom: 8, alignItems: 'center' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 8, overflow: 'hidden', flexShrink: 0, background: post.gradient || 'var(--warm-tan)' }}>
                    {post.image && <img src={post.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 9, color: 'var(--light-text)' }}>by {post.userName} · {post.likes.length} likes · {post.comments.length} comments</div>
                  </div>
                  <button onClick={() => { deletePost(post.id); addNotification('Post removed.') }} className="btn btn-danger btn-sm"><Trash2 size={11} /></button>
                </motion.div>
              ))}
