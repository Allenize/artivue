...
export default function ExploreScreen() {
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: '1rem', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch', paddingBottom: 4, flexWrap: 'nowrap', whiteSpace: 'nowrap' }}>
        <style>{`.filter-scroll::-webkit-scrollbar{display:none}`}</style>
        {cats.map(c => (
          <button key={c} onClick={() => setCat(c)} style={{ flexShrink: 0, padding: '5px 13px', borderRadius: 20, fontSize: 10, fontFamily: 'Jost,sans-serif', cursor: 'pointer', border: '1px solid', transition: 'all .2s', background: cat === c ? 'var(--dark-text)' : 'var(--card-bg)', color: cat === c ? 'var(--cream)' : 'var(--mid-text)', borderColor: cat === c ? 'transparent' : 'var(--border)', whiteSpace: 'nowrap' }}>{c}</button>
        ))}
        <div style={{ width: 1, background: 'var(--border)', flexShrink: 0, margin: '0 4px' }} />
        {eras.filter(e => e !== 'All').map(e => (
          <button key={e} onClick={() => setEra(era === e ? 'All' : e)} style={{ flexShrink: 0, padding: '5px 13px', borderRadius: 20, fontSize: 10, fontFamily: 'Jost,sans-serif', cursor: 'pointer', border: '1px solid', transition: 'all .2s', background: era === e ? 'var(--accent-rust)' : 'var(--card-bg)', color: era === e ? '#fff' : 'var(--mid-text)', borderColor: era === e ? 'transparent' : 'var(--border)', whiteSpace: 'nowrap' }}>{e}</button>
        ))}
      </div>
      ...
      {/* Artwork results — fixed image so it never expands */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <AnimatePresence>
          {filteredArtworks.map((art, i) => (
            <motion.div key={art.id}
              style={{ display: 'flex', background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', overflow: 'hidden', cursor: 'pointer', alignItems: 'stretch', minHeight: 90 }}
              ...
              whileHover={{ borderColor: 'var(--accent-rust)' }}>
              {/* Fixed size image — never grows */}
              <div style={{ width: 90, minWidth: 90, maxWidth: 90, alignSelf: 'stretch', overflow: 'hidden', flexShrink: 0 }}>
                {art.image
                  ? <img src={art.image} alt={art.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                  : <div style={{ width: '100%', height: '100%', background: art.gradient || '#ccc', minHeight: 90 }} />}
              </div>
              <div style={{ flex: 1, padding: '10px 10px 10px 12px', minWidth: 0, overflow: 'hidden' }}>
                <div style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 16, fontStyle: 'italic', color: 'var(--dark-text)', lineHeight: 1.25, marginBottom: 3, wordBreak: 'break-word' }}>{art.title}</div>
